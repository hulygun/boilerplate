...
class UserInterface(UserBaseDBIterface):
    user = DBUser
    rpc = aiohttp_rpc.JsonRpcClient(os.getenv('RPC_URL'))
    ...
    async def generate_sign(self, user: User, expiries: int) -> str:
    ...
    async def get_tokens(self, user: User, fingerprint: str) -> UserTokens:
    ...
    async def send_message(self, message_type: int, recipient: User, extra: Dict = None) -> None:
        ...
        async with self.rpc as rpc:
            return await rpc.send_message(
                backend=os.getenv('FILE_NOTIFICATION'),
    ...
    async def uuid_by_sign(self, sign: UUID) -> Optional[UUID]:
        """Get user_uuid by sign"""
        return self.redis(REDIS_SIGN_DB).get(sign)

    async def verificate_password(self, user: User, password: str) -> bool:
